...
public class IdentityServerDataSeeder : ITransientDependency
{
    ...
    private async Task CreateApiResourcesAsync()
    {
        ...
        await CreateApiResourceAsync("CharacterService", commonApiUserClaims);
    }
    ...
    private async Task CreateApiScopesAsync()
    {
        ...
        await CreateApiScopeAsync("CharacterService");
    }

    private async Task CreateSwaggerClientsAsync()
    {
        await CreateSwaggerClientAsync("WebGateway", new[] { "AuthServer", "IdentityService", "AdministrationService", "SaasService", "ProductService","CharacterService" });
        await CreateSwaggerClientAsync("PublicWebGateway", new[] { "ProductService","CharacterService" });
    }
    ...
    private async Task CreateClientsAsync()
    {
        ...
        var webClientRootUrl =
            _configuration["IdentityServerClients:RickAndMortyApi_Web:RootUrl"].EnsureEndsWith('/');
        await CreateClientAsync(
            name: "RickAndMortyApi_Web",
        ...
        var blazorClientRootUrl =
            _configuration["IdentityServerClients:RickAndMortyApi_Blazor:RootUrl"].EnsureEndsWith('/');
        await CreateClientAsync(
            name: "RickAndMortyApi_Blazor",
            scopes: commonScopes.Union(new[]
            {
                    "AuthServer",
                    "IdentityService",
        ...
        var blazorServerClientRootUrl =
            _configuration["IdentityServerClients:RickAndMortyApi_BlazorServer:RootUrl"].EnsureEndsWith('/');
        await CreateClientAsync(
            name: "RickAndMortyApi_BlazorServer",
            scopes: commonScopes.Union(new[]
            {
                    "AuthServer",
                    "IdentityService",
        ...
        //Public Web Client
        var publicWebClientRootUrl = _configuration["IdentityServerClients:RickAndMortyApi_PublicWeb:RootUrl"]
            .EnsureEndsWith('/');
        await CreateClientAsync(
            name: "RickAndMortyApi_PublicWeb",
            scopes: commonScopes.Union(new[]
            {
                    "AdministrationService",
                    "ProductService",
        ...
        var angularClientRootUrl =
            _configuration["IdentityServerClients:RickAndMortyApi_Angular:RootUrl"].TrimEnd('/');
        await CreateClientAsync(
            name: "RickAndMortyApi_Angular",
            scopes: commonScopes.Union(new[]
            {
                    "AuthServer",
                    "IdentityService",
        ...
        await CreateClientAsync(
            name: "RickAndMortyApi_AdministrationService",
            scopes: commonScopes.Union(new[]
            {
                    "IdentityService"
            }),
